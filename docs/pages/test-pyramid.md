...
- see: [DevOps](/pages/devops)
- see: [CI/CD](/pages/ci-cd)

As the amount and sophistication of software grows, software must be delivered faster, without sacrificing quality.

[Continuous Delivery](/pages/ci-cd#continuous-delivery) allows you to deliver software to production at any time.
...
## Unit Test
The foundation of your test suite is made up of [unit tests](/pages/unit-testing).
...
- see: TDD [Mocks / Mocking](/pages/tdd-test-driven-development#mocks--mocking)

_Mocks_ and _Stubs_ are two different kinds of [Test Doubles](/pages/bdd-behavior-driven-development#doubles--stubs--stubbing).
...
- see: [OOD: Ignoring Private Methods During Tests](/pages/ood-designing-cost-effective-tests#ignoring-private-methods-during-tests)

You shouldn't test private methods!
They are indicative of a design problem, rather than a scoping problem.
Classes that are too complex violate the [Single Responsibility Principle](/pages/solid#s---single-responsiblity-principle), the S from [SOLID](/pages/solid).

The solution is to split the class and move the private method you wish to test to the public interface of a new class.

## What to Test?
- see: [OOD: Knowing What to Test](/pages/ood-designing-cost-effective-tests#knowing-what-to-test)
...
Other mnemonics to remember, taken from [BDD - Behavior Driven Development](/pages/bdd-behavior-driven-development):
- "Arrange, Act, Assert"
- "Given, When, Then"
...
- see: [Unit Test](/pages/unit-testing)
...
[Microservices architecture](/pages/microservice-architecture) requires you to interact with many
different services using APIs, commonly using one of the following:
- REST and JSON via HTTPS
- [RPC -Remote Procedure Call](/pages/rpc-remote-procedure-call) using something like gRPC
...
See: [Centralised QA Team As Antipattern](/pages/qa#centralised-qa-team-as-antipattern)
...
This falls under [BDD - Behavior Driven Development](/pages/bdd-behavior-driven-development), where you can use tools like
...
Tests should run in [Deployment Pipelines](/pages/ci-cd#deployment-pipeline), as part of [CI/CD](/pages/ci-cd).
